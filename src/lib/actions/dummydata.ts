...
import { UserStatus, VolunteerStatus,Gender,CitizenshipType,EmploymentStatus } from '@/models/types';
...
function generateRandomUserData(): UserData {
  ...
  function generateRandomVolunteerData(userId: mongoose.Types.ObjectId): VolunteerData {
    return {
      user: userId,
      ...
      dateOfBirth:faker.date.past({ years: 20, refDate: '2020-01-01T00:00:00.000Z' }),
  ...
  async function generateAndSaveDummyData() {
    ...
    for (let i = 0; i < 10; i++) { // Generate 100 users
      ...
      if (!newUser.isAdmin) { // If not admin, also create a volunteer
