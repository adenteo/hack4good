...
export async function addActivity(activityData: z.infer<typeof activityFormSchema>) {
  // Create a new Activity document
  await connectToDB();
  const existingActivity = await Activity.findOne({ title: activityData.title});
  if (existingActivity) {
    return {
