        ...
        const activitiesWithVolunteers = await Activity.aggregate([
            {
              ...
              $project: {
                ...
                'title': 1, 
                'numHours': 1,
                'startTime': { $dateToString: { format: "%Y-%m-%dT%H:%M:%S.%LZ", date: "$startTime" } }, 
                'endTime': { $dateToString: { format: "%Y-%m-%dT%H:%M:%S.%LZ", date: "$endTime" } }, 
                'tags': 1, 
...
export async function saveActivitiesToCSV(activitiesWithVolunteers: any[], filePath: string): Promise<void> {
    try {
        // Convert JSON to CSV
